...
/** Client-side validation before the API call to give instant feedback. */
...
function RegisterPage() {
  ...
  const handleAvatarChange = (e) => {
    ...
    if (!file) { setAvatarFile(null); setAvatarPreview(null); setAvatarError(''); return; }

    // Client-side type check before sending to server
    ...
    <div className="mx-auto max-w-sm">
      ...
      >
        ...
        <FormInput id="username" name="username" label="Username" type="text"
          ...
          placeholder="janedoe" autoComplete="username" error={fieldErrors.username} />

        <FormInput id="email" name="email" label="University Email" type="email"
          value={fields.email} onChange={handleChange}
          placeholder="you@university.edu" autoComplete="email" error={fieldErrors.email} />

        <FormInput id="password" name="password" label="Password" type="password"
          value={fields.password} onChange={handleChange}
          placeholder="Min. 8 characters" autoComplete="new-password" error={fieldErrors.password} />

        <FormInput id="confirmPassword" name="confirmPassword" label="Confirm Password" type="password"
          value={fields.confirmPassword} onChange={handleChange}
          placeholder="••••••••" autoComplete="new-password" error={fieldErrors.confirmPassword} />
          ...
          <div className="flex items-center gap-3">
            {avatarPreview && (
              <img
                src={avatarPreview}
                alt="Preview"
                className="h-12 w-12 rounded-full object-cover border-2 border-brand-300"
              />
            )}
            <input
              type="file"
